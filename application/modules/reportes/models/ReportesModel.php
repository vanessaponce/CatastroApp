...
class AdministracionModel extends CI_Model
{
    ...
    public function recortarTexto($Array)
    {
        $texto = str_replace('STRING: "', '', implode($Array));

        if (strstr(implode($Array), 'Hex')) {
            $texto = str_replace('Hex-STRING: ', '', implode($Array));
    ...
    public function recortarCantidad($Array)
    {
        $CantidadR = intval(str_replace('INTEGER: ', '', implode($Array)));
        if ($CantidadR == 0) {
            $CantidadR = '';
        }
        return $CantidadR;
    ...
    public function soapEquipo($ip)
    {
        /*$curl = curl_init();
        $soapEquipo = array();

        curl_setopt_array($curl, array(
        CURLOPT_URL => "http://" . $ip . "/ssm/Management/Anonymous/StatusConfig",
        ...
        CURLOPT_POSTFIELDS => "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\r\n<soap:Envelope xmlns:soap=\"http://schemas.xmlsoap.org/soap/envelope/\">\r\n   <soap:Body>\r\n      <cfg:GetAttribute xmlns:cfg=\"http://www.fujixerox.co.jp/2003/12/ssm/management/statusConfig\">\r\n         <cfg:Object name=\"urn:fujixerox:names:ssm:1.0:management:MarkerSupply\" offset=\"0\" >\r\n        \t<Attribute name=\"Name\"></Attribute>\r\n        \t<Attribute name=\"Remaining\"></Attribute>\r\n        \t<Attribute name=\"PageRemaining\"></Attribute>\r\n         </cfg:Object>\r\n         <cfg:Object name=\"urn:fujixerox:names:ssm:1.0:management:ProductName\">\r\n        \t<Attribute name=\"TradeName\"></Attribute>\r\n        </cfg:Object>\r\n         <cfg:Object name=\"urn:fujixerox:names:ssm:1.0:management:productInformation\" offset=\"0\" >\r\n         <Attribute name=\"serialNumber\"></Attribute>\t\r\n         \t</cfg:Object>\r\n         <cfg:Object name=\"urn:fujixerox:names:ssm:1.0:management:TotalUsageCounter\" offset=\"0\" >\r\n         \t<Attribute name=\"Impressions\"></Attribute>\r\n         </cfg:Object>\t\r\n      </cfg:GetAttribute>\r\n   </soap:Body>\r\n</soap:Envelope>",
        ...
        array_push($soapEquipo, 'Offline');
        return $soapEquipo;
        }

        $arr = array();
        if ($err) {
        log_message('error', 'Error curl: ' . $err);
        } else {
        if (count($xml) != 2) {
        if ($xml->envBody->GetAttributeResponse) {
        foreach ($xml->envBody->GetAttributeResponse->Object as $row) {
        array_push($arr, $row->Attribute);
        }
        }
        for ($i = 0; $i < count($arr); $i++) {
        if ($arr[$i][0] == 'TONER_K') {
        array_push($soapEquipo, strval($arr[$i][0]), strval($arr[$i][1]), strval($arr[$i][2]));
        }
        if ($arr[$i][0] == 'DRUM_K') {
        array_push($soapEquipo, strval($arr[$i][0]), strval($arr[$i][1]), strval($arr[$i][2]));
        }
        if ($this->xml_attribute($arr[$i][0], 'name') == 'TradeName') {
        array_push($soapEquipo, strval($arr[$i][0]));
        }
        if ($this->xml_attribute($arr[$i][0], 'name') == 'serialNumber') {
        array_push($soapEquipo, strval($arr[$i][0]));
        }
        if ($this->xml_attribute($arr[$i][0], 'name') == 'Impressions') {
        array_push($soapEquipo, strval($arr[$i][0]));
        }
        }
        return $soapEquipo;
        ...
        $suministroPorcentaje = array();
        $suministroNombre = array();
        $suministroInfo = array();
        $suministro = array();
        $bandejaInfo = array();
        $bandeja = array();

        if (!$session->walk("iso.3.6.1.2.1.1.1.0")) {
            array_push($datos, 'Offline');
        } else {
            array_push($suministroInfo, $this->recortarTexto($session->walk("iso.3.6.1.2.1.1.1.0")), $this->recortarTexto($session->walk("iso.3.6.1.2.1.43.5.1.1.17.1")), intval(str_replace('Counter32: ', '', implode($session->walk("iso.3.6.1.2.1.43.10.2.1.4.1.1")))), $ip);

            for ($i = 1; $i < 30; $i++) {
                if (($this->recortarTexto($session->walk('iso.3.6.1.2.1.43.11.1.1.6.1.' . $i . ''))) != '') {
                    if (($this->recortarCantidad($session->walk('iso.3.6.1.2.1.43.11.1.1.9.1.' . $i . ''))) < 0) {
                        $porcentaje = 'Ok';
                    } else {
                        $porcentaje = (($this->recortarCantidad($session->walk('iso.3.6.1.2.1.43.11.1.1.9.1.' . $i . ''))) * 100) / ($this->recortarCantidad($session->walk('iso.3.6.1.2.1.43.11.1.1.8.1.' . $i . '')));
                        $porcentaje = number_format($porcentaje, 0, '.', '');
                    }
                    array_push($suministroNombre, $this->recortarTexto($session->walk('iso.3.6.1.2.1.43.11.1.1.6.1.' . $i . '')));
                    array_push($suministroPorcentaje, $porcentaje);
                }
                if (($this->recortarTexto($session->walk('iso.3.6.1.2.1.43.8.2.1.18.1.' . $i . ''))) != '') {
                    array_push($bandejaInfo, $this->recortarTexto($session->walk('iso.3.6.1.2.1.43.8.2.1.18.1.' . $i . '')));
                    if (($this->recortarCantidad($session->walk('iso.3.6.1.2.1.43.8.2.1.10.1.' . $i . ''))) == 0) {
                        array_push($bandeja, 'Vacía');
                    } elseif (($this->recortarCantidad($session->walk('iso.3.6.1.2.1.43.8.2.1.10.1.' . $i . ''))) == '-3') {
                        array_push($bandeja, 'Ok');
                    } elseif (($this->recortarCantidad($session->walk('iso.3.6.1.2.1.43.8.2.1.10.1.' . $i . ''))) == '-2') {
                        array_push($bandeja, 'No disponible');
                    } else {
                        array_push($bandeja, number_format((($this->recortarCantidad($session->walk('iso.3.6.1.2.1.43.8.2.1.10.1.' . $i . ''))) * 100) / ($this->recortarCantidad($session->walk('iso.3.6.1.2.1.43.8.2.1.9.1.' . $i . ''))), 0, '.', ''));
                    }
                }
            }
            array_push($suministro, $suministroInfo, $suministroNombre, $suministroPorcentaje, $bandejaInfo, $bandeja);
    ...
    public function soapEquipoContador($ip)
    {
        $session = new SNMP(SNMP::VERSION_1, $ip, "public");
        $contadorR;
        error_reporting(0);
        if (!$session->walk("iso.3.6.1.2.1.1.1.0")) {
            ...
            $contador = $session->walk("iso.3.6.1.2.1.43.10.2.1.4.1.1"); //contador total
            $contadorR = intval(str_replace('Counter32: ', '', implode($contador)));
        }
        $session->close();
        return $contadorR;
    }
